...
const usePropuestaApi = () => {
  ...
  const obtenerPropuestaById = (id: string) =>
    fetchData({ url: `${PROPUESTA}/${id}`, methodo: httpMethod.GET, adapter: propuestaAdapter });

  const crearPropuesta = (data: formValuesPropuesta, libros: string[]) =>
    fetchData({ url: PROPUESTA, methodo: httpMethod.POST, bodyData: JSON.stringify(propuestaDtoAdapter(data, libros)), adapter: propuestaAdapter });

  const editarPropuesta = (data: formValuesPropuesta, id: string, libros:string[]) =>
    fetchData({ url: `${PROPUESTA}/${id}`, methodo: httpMethod.PUT, bodyData: JSON.stringify(propuestaDtoAdapter(data,libros)), adapter: propuestaAdapter });
