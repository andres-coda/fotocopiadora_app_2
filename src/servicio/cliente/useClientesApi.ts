...
const useClientesApi = () => {
  ...
  const obtenerClientesBusqueda = ({ query, limite, pagina }: BusquedaApiProp) => {
    fetchData({ url: `${CLIENTE}?q=${query.trimEnd()}&limite=${limite ?? limiteDefecto}&pagina=${pagina ?? 1}`, methodo: httpMethod.GET, adapter: clienteAdapter });
  }

  const obtenerClientes = () =>
    fetchData({ url: `${CLIENTE}?limite=${limiteDefecto}`, methodo: httpMethod.GET, adapter: clienteAdapter });
